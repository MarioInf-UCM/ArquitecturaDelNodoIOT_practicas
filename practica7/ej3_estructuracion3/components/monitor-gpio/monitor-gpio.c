...
#define OTA_BUTTON_GPIO_PORT CONFIG_OTA_BUTTON_GPIO_PORT
...
esp_event_loop_handle_t monitor_gpio_init()
{
    ...
    gpio_reset_pin(BUTTON_GPIO_PORT);
    gpio_set_direction(BUTTON_GPIO_PORT, GPIO_MODE_INPUT);
    gpio_set_pull_mode(BUTTON_GPIO_PORT, GPIO_PULLUP_PULLDOWN);
...
static void gpio_timer_callback(void *arg)
{
    ...
    if (gpio_get_level(BUTTON_GPIO_PORT)){
        ESP_LOGI(TAG, "Botón GPIO %d presionado", BUTTON_GPIO_PORT);
