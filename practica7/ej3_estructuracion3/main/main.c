...
void app_main(){
    ...
    ESP_LOGI(TAG, "Comenzando ejecución.\n\n");
    ESP_LOGI(TAG, "******************************");
    ESP_LOGI(TAG, "*BIENVENIDOS A LA VERSIÓN 1.0*");
    ESP_LOGI(TAG, "******************************\n\n");
   ...
   // Inicialización del comoponentes Mock Flas - FIN
    ...
    result = esp_event_handler_register_with(loop_monitor_gpio, MONITOR_GPIO, MONITOR_OTA_GPIO_BUTTON_PRESSED, task_monitor_gpio_handler, loop_monitor_gpio);
    if (result != ESP_OK){
        ESP_LOGE(TAG, "ERROR..: No se pudo registrar la manejadora del evento MONITOR_GPIO - MONITOR_OTA_GPIO_BUTTON_PRESSED.");
        vTaskDelete(NULL);
    }
    ...
    // Inicialización del comoponentes OTA service - FIN
    //*************************************************************
...
static void task_monitor_gpio_handler(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
    ...
    if (id == MONITOR_GPIO_BUTTON_PRESSED){
        ESP_LOGW(TAG, "Botón pulsado, accediendo al modo consola");
...
static void task_OTA_service_handler(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
    ...
    if (id == OTA_SERVICE_FAIL_EVENT){
        ESP_LOGE(TAG, "Se ha detectado un error al intentar actualizar el firmware. Operación cancelada");
    }
    else{
        ESP_LOGE(TAG, "ERROR..: ID de evento desconocido");
        return;
