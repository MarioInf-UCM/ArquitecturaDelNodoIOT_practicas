                   ...
                   const size_t nbytes)
{
    ...
    i2c_master_write_byte(cmd, SI7021_I2C_ADDR << 1 | I2C_MASTER_READ, ACK_CHECK_EN);
...
esp_err_t _getSensorReading_withCRC(const i2c_port_t i2c_num, const uint8_t command, float *output, bool *crcResult, bool verboseFlag, float (*fn)(const uint16_t)){
    ...
    vTaskDelay(100 / portTICK_PERIOD_MS);
    uint8_t buf[3];
    ret = _readResponseBytes(i2c_num, buf, 3);
    if (ret != ESP_OK){
        return ret;
    }
...
esp_err_t readTemperature_withCRC(const i2c_port_t i2c_num, float *temperature, bool *crcResult, bool verboseFlag){
    ...
    esp_err_t ret = _getSensorReading_withCRC(i2c_num, SI7021_READ_TEMP, temperature, crcResult, verboseFlag, &_temp_code_to_celsius);
